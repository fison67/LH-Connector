...
metadata {
	definition (name: "Samsung Smart DoorLock Zwave", namespace: "streamorange58819", fison67: "SmartThings", genericHandler: "Z-Wave", mnmn: "fison67",  vid:"0e71013a-70b3-31c2-b210-db9e40ce2b3c") {
		capability "Lock"
		capability "Actuator"
        capability "streamorange58819.unlocktype"
        capability "streamorange58819.unlockperson"
        capability "streamorange58819.mpassword"
        capability "streamorange58819.pwdcount"
...
// Samsung
		fingerprint mfr:"022E", prod:"0001", model:"0001", deviceJoinName: "Samsung Door Lock" // SHP-DS705, SHP-DHP728, SHP-DHP525 //Samsung Digital Lock
// KeyWe
...
// Philia
    ...
    preferences {
		input name:"leftCount", type:"number", title:"임시 비밀번호 사용 가능 카운트", defaultValue: 1, displayDuringSetup: true
	}
}
...
def getMasterKey(){
	return device.currentValue("maxCodes")
}
...
def processUnlockPerson(data){
	log.info data
	def person = state[data.id.toString()]
    if(!person){
        person = "Unknown"
    }
    ...
    sendEvent(name: "unlocktype", value: data.unlockType)
    sendEvent(name:"codeid", value: data.id.toString())
    sendEvent(name: "lockTypeId", value: data.unlockTypeId)
}
...
// DoorLockOperationReport is called when trying to read the lock state or when the lock is locked/unlocked from the DTH or the smart app
    ...
    if(device.currentValue("pwdcount") == 0 && device.currentValue("mpassword") != null){
		runIn(3, "setMasterPasswordCount")
    }
...
private def handleAccessAlarmReport(cmd) {
	...
	case 6: // Unlocked with keypad
        if (cmd.eventParameter || cmd.alarmLevel) {
            ...
            map.data = [ codeId: codeID as String, usedCode: codeID, codeName: codeName, method: "keypad" ]
            if(codeID == masterKey){
            	runIn(5, "processMasterPasswordCount", [overwrite: true, forceForLocallyExecuting: true])
            }
        }
		break
    case 7:
	...
	case 0xF: // Duplicate Pin-code error
        if (cmd.eventParameter || cmd.alarmLevel) {
            ...
            map = [ name: "codeChanged", value: "$codeID failed", descriptionText: "User code is duplicate and not added", isStateChange: true, data: [isCodeDuplicate: true] ]
        }
	break
	...
	case 0x12: // Master code changed
        ...
        //set code....
        setMasterPassword(device.currentValue("mpassword"))
		break
    ...
    try{
    	if(map != null && map.name == "lock" && map.value == "unlocked"){
        	def codeId = readCodeSlotId(cmd)
            def unlockType = ""
            def unlockTypeId = 100
            if (cmd.zwaveAlarmEvent == 6) {
                if(codeId == masterKey || codeId == 0){
                	unlockType = getTextString("usingKeypad")
                    unlockTypeId = 0
                }else{
                	unlockType = getTextString("usingKeyCard")
                    unlockTypeId = 3
                }
            } else if (cmd.zwaveAlarmEvent == 2) {
                unlockType = getTextString("fromInside")
                unlockTypeId = 2
            } else if (cmd.zwaveAlarmEvent == 100) {
                unlockType = getTextString("usingFingerprint")
                unlockTypeId = 4
            } else if (cmd.zwaveAlarmEvent == 4) {
                unlockType = getTextString("usingBluetooth")
                unlockTypeId = 1
            }
            processUnlockPerson(["id":codeId, "lock":map.value, "unlockType": unlockType, "unlockTypeId": unlockTypeId])
        }else if(map != null && map.name == "lock" && map.value == "locked"){
        	def lockType = getTextString("autoLock")
        	if (cmd.zwaveAlarmEvent == 3) {
            	lockType = getTextString("usingBluetooth")
            }
        	processUnlockPerson(["id":255, "lock":map.value, "unlockType":lockType , "unlockTypeId": 100])
        }
    }catch(err){
    	log.error err
        log.error map
...
// cmd.userIdentifier seems to be an int primitive type
    ...
    if(/*cmd.userIdStatus == 1 &&*/ cmd.userIdentifier == masterKey){
    	sendEvent(name: "mpassword", value: cmd.code)
    }
    def lockCodes = loadLockCodes()
    def map = [ name: "codeChanged", isStateChange: true ]
    def deviceName = device.displayName
    def userIdStatus = cmd.userIdStatus
    if (userIdStatus == UserCodeReport.USER_ID_STATUS_OCCUPIED || (userIdStatus == UserCodeReport.USER_ID_STATUS_STATUS_NOT_AVAILABLE && cmd.user)) {
		def codeName
		...
		map = [ name: "codeChanged", value: "$codeID failed", descriptionText: "User code is not added", isStateChange: true, data: [ lockName: deviceName, isCodeDuplicate: true] ]
	} else {
		...
		if (codeID == "0" && userIdStatus == UserCodeReport.USER_ID_STATUS_AVAILABLE_NOT_SET && isSchlageLock()) {
			...
			result << allCodesDeletedEvent()
			map = [ name: "codeChanged", value: "all deleted", descriptionText: "Deleted all user codes", isStateChange: true, data: [ lockName: deviceName, notify: true, notificationText: "Deleted all user codes in $deviceName at ${location.name}"] ]
			lockCodes = [:]
    ...
    if (codeID.toInteger() == state.checkCode) {  // reloadAllCodes() was called, keep requesting the codes in order
    	if (state.checkCode + 1 > state.codes || state.checkCode > 30) {
...
def processMasterPasswordCount(){
	def count = device.currentValue("pwdcount")
    if(count == 0){
    	return
    }
    count--
    setMasterPasswordCount(count)
}

def setMasterPasswordCount(count=0){
    sendEvent(name: "pwdcount", value: count)
    if(count <= 0){
		sendHubCommand(secure(zwave.userCodeV1.userCodeSet(userIdentifier:masterKey, userIdStatus:0)))
		runIn(3, "requestCurrentMasterInfo")
    }
}

def requestCurrentMasterInfo(){
	log.debug "requestCurrentMasterInfo[${masterKey}]"
	sendHubCommand(secure(zwave.userCodeV1.userCodeGet(userIdentifier:masterKey)))
}

def setMasterPassword(password){
	
	def lockCodes = loadLockCodes()
    setMasterPasswordCount(leftCount == null ? 1 : leftCount)
    
	def cmds = []
	cmds << setCode(masterKey, password.toString(), "guest")
    cmds << "delay 3000"
    cmds << requestCode(masterKey)
    cmds
}
...
def refresh() {
    ...
    def cmds = secureSequence([zwave.doorLockV1.doorLockOperationGet(), zwave.batteryV1.batteryGet(), zwave.userCodeV1.usersNumberGet()])
...
def reloadAllCodes() {
    ...
    log.trace "checkCode: ${state.checkCode}"
	def cmds = []
	...
	if (!state.codes) {
    	...
    	log.debug "maxCodes: ${state.codes}"
...
def setCode(codeID, code, codeName = null) {
	log.debug "setCode >> [$code]" + codeName
    if (!code) {
	...
	def newCodeName = codeName ? codeName : "Code $codeSlot"
    if(oldCodeName != newCodeName){
        lockCodes[codeID.toString()] = newCodeName
        sendEvent(lockCodesEvent(lockCodes))
    }
    
	def strname = (codeName ? codeName : "Code $codeID")
...
def updateCodes(codeSettings) {
	...
	if(codeSettings instanceof String) codeSettings = util.parseJson(codeSettings)
		...
		if (name.startsWith("code")) {
			...
			if (updated && updated.size() >= 4 && updated.size() <= 8) {
				log.debug "Setting code number $n >> ${updated}"
	...
	if (set_cmds) {
        set_cmds << secure(zwave.userCodeV1.userCodeGet(userIdentifier:1))
...
void nameSlot(codeSlot, codeName) {
	...
	sendEvent(lockCodesEvent(lockCodes))
	sendEvent(name: "codeChanged", value: "$codeSlot renamed", data: [ lockName: deviceName, notify: false, notificationText: "Renamed \"$oldCodeName\" to \"$newCodeName\" in $deviceName at ${location.name}" ], descriptionText: "Renamed \"$oldCodeName\" to \"$newCodeName\"", displayed: true, isStateChange: true)
}
/**
...
def deviceName = device.displayName
	lockCodes.each { id, code ->
		result << createEvent(name: "codeReport", value: id, data: [ code: "" ], descriptionText: "code $id was deleted", displayed: false, isStateChange: true)
		def codeName = code
		result << createEvent(name: "codeChanged", value: "$id deleted", data: [ codeName: codeName, lockName: deviceName, notify: true, notificationText: "Deleted \"$codeName\" in $deviceName at ${location.name}" ], descriptionText: "Deleted \"$codeName\"", displayed: true, isStateChange: true)
...
def generatesDoorLockOperationReportBeforeAlarmReport() {
	...
	if(isYaleLock() && (("0007" == zwaveInfo.prod && "0001" == zwaveInfo.model) || ("6600" == zwaveInfo.prod && "0002" == zwaveInfo.model) )) {
